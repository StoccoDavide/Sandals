...
public:
  ...
  MatrixJF Jf_x(VectorF const & x, Real const /*t*/) const override {
    ...
    Jf_x(1, 0) = 0.5;
    Jf_x(1, 2) = -0.5;
    Real N{0.9 - 1000.0 * (x(2) - x(4)) - 0.5 * std::pow(x(2), 2.0) + 0.5 * x(0) * x(2)};
...
public:
  ...
  VectorF F(VectorF const & x, VectorF const & x_dot, Real const /*t*/) const override
  {
    VectorF F;
    F <<
      x_dot(0) - 0.5*x(0)*(x(2) - x(0))/x(1),
      x_dot(1) + 0.5*(x(2) - x(0)),
      x_dot(2) - (0.9 - 1000*(x(2) - x(4)) - 0.5*x(2)*(x(2) - x(0)))/x(3),
      x_dot(3) - 0.5*(x(2) - x(0)),
      x_dot(4) - 100*(x(2) - x(4));
    return F;
  }
...
class Shampine1Problem : public Problem<Real, 5, 0, Integrator>
{
public:
  using typename Problem<Real, 5, 0, Integrator>::SystemPtr;
  using typename Problem<Real, 5, 0, Integrator>::IntegratorPtr;
  using typename Problem<Real, 5, 0, Integrator>::SolutionPtr;
  using typename Problem<Real, 5, 0, Integrator>::VectorF;
  using typename Problem<Real, 5, 0, Integrator>::MatrixJF;
  using VectorX = Eigen::Vector<Real, Eigen::Dynamic>;
  using MatrixX = Eigen::Matrix<Real, 5, Eigen::Dynamic>;

  Shampine1Problem()
    : Problem<Real, 5, 0, Integrator>("Shampine1Problem", std::make_unique<System>(),
  ...
  static VectorF analytical_solution(Real const /*t*/) {return VectorF::Zero();}

  static MatrixX analytical_solution(VectorX const & t) {return MatrixX::Zero(5, t.size());}

  static VectorF guess(Real const t) {
    VectorF x;
    x <<
      1.0,
      1.0,
  ...
  static MatrixX guess(VectorX const & t) {
    MatrixX x(5, t.size());
