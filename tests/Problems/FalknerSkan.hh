...
class FalknerSkanProblem : public Problem<Real, 3, 0, Integrator>
{
public:
  using typename Problem<Real, 3, 0, Integrator>::SystemPtr;
  using typename Problem<Real, 3, 0, Integrator>::IntegratorPtr;
  using typename Problem<Real, 3, 0, Integrator>::SolutionPtr;
  using typename Problem<Real, 3, 0, Integrator>::VectorF;
  using typename Problem<Real, 3, 0, Integrator>::MatrixJF;
  using VectorX = Eigen::Vector<Real, Eigen::Dynamic>;
  using MatrixX = Eigen::Matrix<Real, 3, Eigen::Dynamic>;

  FalknerSkanProblem()
    : Problem<Real, 3, 0, Integrator>("FalknerSkanProblem", std::make_unique<System>(),
  ...
  MatrixX analytical_solution(VectorX const & t) const {return MatrixX::Zero(3, t.size());}
