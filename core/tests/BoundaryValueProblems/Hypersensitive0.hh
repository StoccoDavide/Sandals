...
class Hypersensitive0Explicit : public Explicit<Real, 4, 0> {
 public:
  ...
  VectorF f(const VectorF &x, const Real /*t*/) const override {
    VectorF f;
    f << x(1), -x(0) - x(0) * x(0) * x(0) - x(3),
        -x(0) + x(3) * (1.0 + 3.0 * x(0) * x(0)), -x(1) - x(2);
    return f;
  }
...
class Hypersensitive0Implicit : public Implicit<Real, 4, 0> {
 public:
  ...
  VectorF F(const VectorF &x,
    ...
    VectorF F;
    F << x_dot(0) - x(1), x_dot(1) + x(0) + x(0) * x(0) * x(0) + x(3),
...
class Hypersensitive0Problem : public BVP<Real, 4, 0, Integrator> {
 public:
  ...
  Hypersensitive0Problem()
      : BVP<Real, 4, 0, Integrator>("Hypersensitive0Problem",
  ...
  VectorF b(const VectorF &x_ini, const VectorF &x_end) const override {
    VectorF b;
    b << x_ini(0) - 1.0, x_ini(1), x_end(0) - 0.75, x_end(1);
    return b;
  }
