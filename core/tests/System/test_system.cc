...
int main(int argc, char **argv) {
  ...
  Fehlberg45<Real, 2, 0>::Time time =
      Eigen::VectorXd::LinSpaced(1000, 0.0, 10.0);

  SinCosImplicit system_implicit;
  Fehlberg45<Real, 2, 0> integrator_implicit(
      std::make_unique<SinCosImplicit<Real>>(system_implicit));
  ...
  integrator_implicit.adaptive_solve(time,
                                     system_implicit.ics(),
                                     solution_implicit_adaptive);

#ifdef SANDALS_ENABLE_PLOTTING
  TCanvas *c1a = new TCanvas("c1a", "Oscillator Implicit", 1200, 800);
  TGraph *g1a_x0 =
      to_TGraph(solution_implicit.eigen_t(), solution_implicit.eigen_x(0));
  TGraph *g1a_x1 =
      to_TGraph(solution_implicit.eigen_t(), solution_implicit.eigen_x(1));
  TGraph *g1a_x0a = to_TGraph(solution_implicit_adaptive.eigen_t(),
                              solution_implicit_adaptive.eigen_x(0));
  TGraph *g1a_x1a = to_TGraph(solution_implicit_adaptive.eigen_t(),
                              solution_implicit_adaptive.eigen_x(1));

  auto colors = matlab_lines_colormap();
  g1a_x0->SetLineColor(colors[0]);
  g1a_x0->Draw("AL");
  g1a_x1->SetLineColor(colors[0]);
  g1a_x1->Draw("L SAME");
  g1a_x0a->SetLineColor(colors[1]);
  g1a_x0a->SetLineStyle(9);
  g1a_x0a->Draw("L SAME");
  g1a_x1a->SetLineColor(colors[1]);
  g1a_x1a->SetLineStyle(9);
  g1a_x1a->Draw("L SAME");
  ...
  Fehlberg45<Real, 2, 0> integrator_explicit(
      std::make_unique<SinCosExplicit<Real>>(system_explicit));
  ...
  integrator_explicit.adaptive_solve(time,
                                     system_explicit.ics(),
                                     solution_explicit_adaptive);

#ifdef SANDALS_ENABLE_PLOTTING
  TCanvas *c1b = new TCanvas("c1b", "Sine Explicit", 1200, 800);
  TGraph *g1b_x0 =
      to_TGraph(solution_explicit.eigen_t(), solution_explicit.eigen_x(0));
  TGraph *g1b_x1 =
      to_TGraph(solution_explicit.eigen_t(), solution_explicit.eigen_x(1));
  TGraph *g1b_x0a = to_TGraph(solution_explicit_adaptive.eigen_t(),
                              solution_explicit_adaptive.eigen_x(0));
  TGraph *g1b_x1a = to_TGraph(solution_explicit_adaptive.eigen_t(),
                              solution_explicit_adaptive.eigen_x(1));

  g1b_x0->SetLineColor(colors[0]);
  g1b_x0->Draw("AL");
  g1b_x1->SetLineColor(colors[0]);
  g1b_x1->Draw("L SAME");
  g1b_x0a->SetLineColor(colors[1]);
  g1b_x0a->SetLineStyle(9);
  g1b_x0a->Draw("L SAME");
  g1b_x1a->SetLineColor(colors[1]);
  g1b_x1a->SetLineStyle(9);
  g1b_x1a->Draw("L SAME");
  ...
  Fehlberg45<Real, 2, 0> integrator_semiexplicit(
      std::make_unique<SinCosSemiExplicit<Real>>(system_semiexplicit));
  ...
  integrator_semiexplicit.solve(time,
                                system_semiexplicit.ics(),
                                solution_semiexplicit);
  integrator_semiexplicit.adaptive_solve(time,
                                         system_semiexplicit.ics(),
                                         solution_semiexplicit_adaptive);

#ifdef SANDALS_ENABLE_PLOTTING
  TCanvas *c1c    = new TCanvas("c1c", "Oscillator SemiExplicit", 1200, 800);
  TGraph *g1c_x0  = to_TGraph(solution_semiexplicit.eigen_t(),
                             solution_semiexplicit.eigen_x(0));
  TGraph *g1c_x1  = to_TGraph(solution_semiexplicit.eigen_t(),
                             solution_semiexplicit.eigen_x(1));
  TGraph *g1c_x0a = to_TGraph(solution_semiexplicit_adaptive.eigen_t(),
                              solution_semiexplicit_adaptive.eigen_x(0));
  TGraph *g1c_x1a = to_TGraph(solution_semiexplicit_adaptive.eigen_t(),
                              solution_semiexplicit_adaptive.eigen_x(1));

  g1c_x0->SetLineColor(colors[0]);
  g1c_x0->Draw("AL");
  g1c_x1->SetLineColor(colors[0]);
  g1c_x1->Draw("L SAME");
  g1c_x0a->SetLineColor(colors[1]);
  g1c_x0a->SetLineStyle(9);
  g1c_x0a->Draw("L SAME");
  g1c_x1a->SetLineColor(colors[1]);
  g1c_x1a->SetLineStyle(9);
  g1c_x1a->Draw("L SAME");
