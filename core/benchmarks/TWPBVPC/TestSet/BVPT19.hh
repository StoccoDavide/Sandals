...
#ifndef TESTS_PROBLEMS_BVPT19_HH
#define TESTS_PROBLEMS_BVPT19_HH
...
class BVPT19Explicit : public Explicit<Real, 2, 0> {
 public:
 ...
 public:
  BVPT19Explicit() : Explicit<Real, 2, 0>("BVPT19Explicit") {}
  ~BVPT19Explicit() {}
  ...
  MatrixJF Jf_x(const VectorF &x, const Real t) const override {
    ...
    Jf_x(1, 0) = (pi * std::sin(pi * t / 2.0) * std::exp(2.0 * x(0)) -
                  std::exp(x(0)) * x(1)) /
                 this->m_lambda;
    Jf_x(1, 1) = -std::exp(x(0)) / this->m_lambda;
    return Jf_x;
  }
...
template <typename Real = double>
class BVPT19Implicit : public Implicit<Real, 2, 0> {
 public:
 ...
 public:
  BVPT19Implicit() : Implicit<Real, 2, 0>("BVPT19Implicit") {}
  ~BVPT19Implicit() {}
...
template <typename Real = double>
class BVPT19SemiExplicit : public SemiExplicit<Real, 2, 0> {
 public:
 ...
 public:
  BVPT19SemiExplicit() : SemiExplicit<Real, 2, 0>("BVPT19SemiExplicit") {}
  ~BVPT19SemiExplicit() {}
  ...
  MatrixJB Jb_x(const VectorF &x, const Real t) const override {
    ...
    Jb_x(1, 0) = (pi * std::sin(pi * t / 2.0) * std::exp(2.0 * x(0)) -
                  std::exp(x(0)) * x(1)) /
                 this->m_lambda;
    Jb_x(1, 1) = -std::exp(x(0)) / this->m_lambda;
    return Jb_x;
  }
...
class BVPT19Problem : public BVP<Real, 2, 0, Integrator> {
 public:
  ...
  BVPT19Problem()
      : BVP<Real, 2, 0, Integrator>("BVPT19Problem",
...
#endif  // TESTS_PROBLEMS_BVPT19_HH
