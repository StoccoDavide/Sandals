...
#ifndef TESTS_PROBLEMS_BVPT30_HH
#define TESTS_PROBLEMS_BVPT30_HH
...
class BVPT30Explicit : public Explicit<Real, 2, 0> {
 public:
 ...
 public:
  BVPT30Explicit() : Explicit<Real, 2, 0>("BVPT30Explicit") {}
  ~BVPT30Explicit() {}
  ...
  VectorF f(const VectorF &x, const Real /*t*/) const override {
    VectorF f;
    f << x(1), x(0) * (1.0 - x(1)) / this->m_lambda;
    return f;
  }
  ...
  MatrixJF Jf_x(const VectorF &x, const Real /*t*/) const override {
    ...
    Jf_x(0, 1) = 1.0;
    Jf_x(1, 0) = (1.0 - x(1)) / this->m_lambda;
...
template <typename Real = double>
class BVPT30Implicit : public Implicit<Real, 2, 0> {
 public:
 ...
 public:
  BVPT30Implicit() : Implicit<Real, 2, 0>("BVPT30Implicit") {}
  ~BVPT30Implicit() {}
  ...
  VectorF F(const VectorF &x,
    ...
    VectorF F;
    F << x_dot(0) - x(1), x_dot(1) - x(0) * (1.0 - x(1)) / this->m_lambda;
    return F;
  }
...
template <typename Real = double>
class BVPT30SemiExplicit : public SemiExplicit<Real, 2, 0> {
 public:
 ...
 public:
  BVPT30SemiExplicit() : SemiExplicit<Real, 2, 0>("BVPT30SemiExplicit") {}
  ~BVPT30SemiExplicit() {}
  ...
  VectorB b(const VectorF &x, const Real /*t*/) const override {
    VectorB b;
    b << x(1), x(0) * (1.0 - x(1)) / this->m_lambda;
    return b;
  }
...
class BVPT30Problem : public BVP<Real, 2, 0, Integrator> {
 public:
  ...
  BVPT30Problem()
      : BVP<Real, 2, 0, Integrator>("BVPT30Problem",
  ...
  VectorF b(const VectorF &x_ini, const VectorF &x_end) const override {
    VectorF b;
    b << x_ini(0) + 7.0 / 6.0, x_end(0) - 3.0 / 2.0;
    return b;
  }
...
#endif  // TESTS_PROBLEMS_BVPT30_HH
