...
#ifndef TESTS_PROBLEMS_BVPT5_HH
#define TESTS_PROBLEMS_BVPT5_HH
...
/**
  ...
  $$
  \lambda z^{\prime \prime} = z + t z^{\prime} - (1 + \lambda \pi^2) \cos(\pi t)
  + t \pi \sin(\pi t),
  $$
...
class BVPT5Explicit : public Explicit<Real, 2, 0> {
 public:
 ...
 public:
  BVPT5Explicit() : Explicit<Real, 2, 0>("BVPT5Explicit") {}
  ...
  VectorF f(const VectorF &x, const Real t) const override {
    ...
    VectorF f;
    f << x(1),
        (x(0) + t * x(1) - (1.0 + this->m_lambda * pi * pi) * std::cos(tmp) +
         t * pi * std::sin(tmp)) /
            this->m_lambda;
    return f;
  }
...
template <typename Real = double>
class BVPT5Implicit : public Implicit<Real, 2, 0> {
 public:
 ...
 public:
  BVPT5Implicit() : Implicit<Real, 2, 0>("BVPT5Implicit") {}
...
template <typename Real = double>
class BVPT5SemiExplicit : public SemiExplicit<Real, 2, 0> {
 public:
 ...
 public:
  BVPT5SemiExplicit() : SemiExplicit<Real, 2, 0>("BVPT5SemiExplicit") {}
...
class BVPT5Problem : public BVP<Real, 2, 0, Integrator> {
 public:
 ...
 public:
  BVPT5Problem()
      : BVP<Real, 2, 0, Integrator>("BVPT5Problem",
