...
#ifndef TESTS_PROBLEMS_BVPT22_HH
#define TESTS_PROBLEMS_BVPT22_HH
...
class BVPT22Explicit : public Explicit<Real, 2, 0> {
 public:
 ...
 public:
  BVPT22Explicit() : Explicit<Real, 2, 0>("BVPT22Explicit") {}
  ~BVPT22Explicit() {}
  ...
  VectorF f(const VectorF &x, const Real /*t*/) const override {
    VectorF f;
    f << x(1), -(x(1) + x(0) * x(0)) / this->m_lambda;
    return f;
  }
  ...
  MatrixJF Jf_x(const VectorF &x, const Real /*t*/) const override {
    ...
    Jf_x(0, 1) = 1.0;
    Jf_x(1, 0) = -2.0 * x(0) / this->m_lambda;
...
template <typename Real = double>
class BVPT22Implicit : public Implicit<Real, 2, 0> {
 public:
 ...
 public:
  BVPT22Implicit() : Implicit<Real, 2, 0>("BVPT22Implicit") {}
  ~BVPT22Implicit() {}
  ...
  VectorF F(const VectorF &x,
    ...
    VectorF F;
    F << x_dot(0) - x(1), x_dot(1) + (x(1) + x(0) * x(0)) / this->m_lambda;
    return F;
  }
...
template <typename Real = double>
class BVPT22SemiExplicit : public SemiExplicit<Real, 2, 0> {
 public:
 ...
 public:
  BVPT22SemiExplicit() : SemiExplicit<Real, 2, 0>("BVPT22SemiExplicit") {}
  ~BVPT22SemiExplicit() {}
  ...
  VectorB b(const VectorF &x, const Real /*t*/) const override {
    VectorB b;
    b << x(1), -(x(1) + x(0) * x(0)) / this->m_lambda;
    return b;
  }
  ...
  MatrixJB Jb_x(const VectorF &x, const Real /*t*/) const override {
    ...
    Jb_x(0, 1) = 1.0;
    Jb_x(1, 0) = -2.0 * x(0) / this->m_lambda;
...
class BVPT22Problem : public BVP<Real, 2, 0, Integrator> {
 public:
  ...
  BVPT22Problem()
      : BVP<Real, 2, 0, Integrator>("BVPT22Problem",
...
#endif  // TESTS_PROBLEMS_BVPT22_HH
