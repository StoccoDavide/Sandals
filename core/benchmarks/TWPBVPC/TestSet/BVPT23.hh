...
#ifndef TESTS_PROBLEMS_BVPT23_HH
#define TESTS_PROBLEMS_BVPT23_HH
...
class BVPT23Explicit : public Explicit<Real, 2, 0> {
 public:
 ...
 public:
  BVPT23Explicit() : Explicit<Real, 2, 0>("BVPT23Explicit") {}
  ~BVPT23Explicit() {}
  ...
  VectorF f(const VectorF &x, const Real /*t*/) const override {
    VectorF f;
    f << x(1), std::sinh(x(0) / this->m_lambda) / this->m_lambda;
    return f;
  }
  ...
  MatrixJF Jf_x(const VectorF &x, const Real /*t*/) const override {
    ...
    Jf_x(0, 1) = 1.0;
    Jf_x(1, 0) =
        std::cosh(x(0) / this->m_lambda) / (this->m_lambda * this->m_lambda);
    return Jf_x;
  }
...
template <typename Real = double>
class BVPT23Implicit : public Implicit<Real, 2, 0> {
 public:
 ...
 public:
  BVPT23Implicit() : Implicit<Real, 2, 0>("BVPT23Implicit") {}
  ~BVPT23Implicit() {}
  ...
  VectorF F(const VectorF &x,
    ...
    VectorF F;
    F << x_dot(0) - x(1),
        x_dot(1) - std::sinh(x(0) / this->m_lambda) / this->m_lambda;
    return F;
  }
...
template <typename Real = double>
class BVPT23SemiExplicit : public SemiExplicit<Real, 2, 0> {
 public:
 ...
 public:
  BVPT23SemiExplicit() : SemiExplicit<Real, 2, 0>("BVPT23SemiExplicit") {}
  ~BVPT23SemiExplicit() {}
  ...
  VectorB b(const VectorF &x, const Real /*t*/) const override {
    VectorB b;
    b << x(1), std::sinh(x(0) / this->m_lambda) / this->m_lambda;
    return b;
  }
...
class BVPT23Problem : public BVP<Real, 2, 0, Integrator> {
 public:
  ...
  BVPT23Problem()
      : BVP<Real, 2, 0, Integrator>("BVPT23Problem",
...
#endif  // TESTS_PROBLEMS_BVPT23_HH
