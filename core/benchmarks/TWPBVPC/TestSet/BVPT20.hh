...
#ifndef TESTS_PROBLEMS_BVPT20_HH
#define TESTS_PROBLEMS_BVPT20_HH
...
class BVPT20Explicit : public Explicit<Real, 2, 0> {
 public:
 ...
 public:
  BVPT20Explicit() : Explicit<Real, 2, 0>("BVPT20Explicit") {}
  ~BVPT20Explicit() {}
  ...
  VectorF f(const VectorF &x, const Real /*t*/) const override {
    VectorF f;
    f << x(1), (1.0 - x(1) * x(1)) / this->m_lambda;
    return f;
  }
  ...
  MatrixJF Jf_x(const VectorF &x, const Real /*t*/) const override {
    ...
    Jf_x(0, 1) = 1.0;
    Jf_x(1, 1) = -2.0 * x(1) / this->m_lambda;
    return Jf_x;
  }
...
template <typename Real = double>
class BVPT20Implicit : public Implicit<Real, 2, 0> {
 public:
 ...
 public:
  BVPT20Implicit() : Implicit<Real, 2, 0>("BVPT20Implicit") {}
  ~BVPT20Implicit() {}
  ...
  VectorF F(const VectorF &x,
    ...
    VectorF F;
    F << x_dot(0) - x(1), x_dot(1) - (1.0 - x(1) * x(1)) / this->m_lambda;
    return F;
  }
...
template <typename Real = double>
class BVPT20SemiExplicit : public SemiExplicit<Real, 2, 0> {
 public:
 ...
 public:
  BVPT20SemiExplicit() : SemiExplicit<Real, 2, 0>("BVPT20SemiExplicit") {}
  ~BVPT20SemiExplicit() {}
  ...
  VectorB b(const VectorF &x, const Real /*t*/) const override {
    VectorB b;
    b << x(1), (1.0 - x(1) * x(1)) / this->m_lambda;
    return b;
  }
  ...
  MatrixJB Jb_x(const VectorF &x, const Real /*t*/) const override {
    ...
    Jb_x(0, 1) = 1.0;
    Jb_x(1, 1) = -2.0 * x(1) / this->m_lambda;
    return Jb_x;
  }
...
class BVPT20Problem : public BVP<Real, 2, 0, Integrator> {
 public:
  ...
  BVPT20Problem()
      : BVP<Real, 2, 0, Integrator>("BVPT20Problem",
  ...
  VectorX exact_solution(const VectorX &t) const {
    ...
    for (Integer i{0}; i < t.size(); ++i) {
      x(i) = BVPT20Problem<Real, System, Integrator>::exact_solution(t(i));
    }
    return x;
  }
...
#endif  // TESTS_PROBLEMS_BVPT20_HH
