...
#define PROBLEM_INIT(PROBLEM, INTEGRATOR)                                 \
  PROBLEM##Problem<Real, PROBLEM##Implicit<Real>, INTEGRATOR<Real, 8, 0>> \
      problem;
#endif
...
int main(int argc, char **argv) {
  ...
  PROBLEM_INIT(AlpRiderOCP, RadauIIA5)
  ...
  // Set initial guess
  Eigen::Matrix<Real, 8, Eigen::Dynamic> guess(problem.guess(time));

  // Solve the problems with shooting
  Solution<Real, 8, 0> solution(time.size());
  try {
  ...
  TGraph *graph_x3 = to_TGraph(solution.t, solution.eigen_x(2));
  TGraph *graph_x4 = to_TGraph(solution.t, solution.eigen_x(3));
  TGraph *graph_l1 = to_TGraph(solution.t, solution.eigen_x(4));
  TGraph *graph_l2 = to_TGraph(solution.t, solution.eigen_x(5));
  TGraph *graph_l3 = to_TGraph(solution.t, solution.eigen_x(6));
  TGraph *graph_l4 = to_TGraph(solution.t, solution.eigen_x(7));

  graph_x1->SetLineColor(colors[0]);
  graph_x2->SetLineColor(colors[1]);
  graph_x3->SetLineColor(colors[2]);
  graph_x4->SetLineColor(colors[3]);
  graph_l1->SetLineColor(colors[0]);
  graph_l2->SetLineColor(colors[1]);
  graph_l3->SetLineColor(colors[2]);
  graph_l4->SetLineColor(colors[3]);
  ...
  mg1->GetYaxis()->SetTitle("x1, x2, x3, x4");
  mg1->GetHistogram()->GetYaxis()->SetRangeUser(-4, 4);
  auto legend1 = new TLegend(0.7, 0.7, 0.9, 0.9);
  legend1->AddEntry(graph_x1, "x1", "l");
  legend1->AddEntry(graph_x2, "x2", "l");
  legend1->AddEntry(graph_x3, "x3", "l");
  legend1->AddEntry(graph_x4, "x4", "l");
  legend1->Draw();
