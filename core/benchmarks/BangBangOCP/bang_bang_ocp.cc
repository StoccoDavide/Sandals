...
#define PROBLEM_INIT(PROBLEM, INTEGRATOR)                                 \
  PROBLEM##Problem<Real, PROBLEM##Implicit<Real>, INTEGRATOR<Real, 6, 0>> \
      problem;
#endif
...
int main(int argc, char **argv) {
  ...
  // Set initial guess
  Eigen::Matrix<Real, 6, Eigen::Dynamic> guess(problem.guess(time));

  // Solve the problems with shooting
  Solution<Real, 6, 0> solution(time.size());
  try {
    std::cout << "Solving problem..." << std::endl;
    problem.sigma(1.0);
    problem.lambda(1.0e-3);
  ...
  TGraph *graph_x  = to_TGraph(solution.t, solution.eigen_x(0));
  TGraph *graph_y  = to_TGraph(solution.t, solution.eigen_x(1));
  TGraph *graph_T  = to_TGraph(solution.t, solution.eigen_x(2));
  TGraph *graph_l1 = to_TGraph(solution.t, solution.eigen_x(3));
  TGraph *graph_l2 = to_TGraph(solution.t, solution.eigen_x(4));
  TGraph *graph_l3 = to_TGraph(solution.t, solution.eigen_x(5));

  graph_x->SetLineColor(colors[0]);
  graph_y->SetLineColor(colors[1]);
  graph_T->SetLineColor(colors[2]);
  graph_l1->SetLineColor(colors[3]);
  graph_l2->SetLineColor(colors[0]);
  graph_l3->SetLineColor(colors[1]);
  ...
  legend1->AddEntry(graph_x, "x", "l");
  legend1->AddEntry(graph_y, "y", "l");
  legend1->AddEntry(graph_T, "T", "l");
  legend1->Draw();
