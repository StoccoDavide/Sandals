...
#define SANDALS_BASIC_CONSTANTS(Real)                                          \
  static constexpr Real EPSILON{                                               \
    std::numeric_limits<Real>::epsilon()}; /**< Machine epsilon epsilon static \
                                              constant value. */               \
  static constexpr Real EPSILON_HIGH{                                          \
    1.0e-12}; /**< High precision epsilon static constant value. */            \
  static constexpr Real EPSILON_MEDIUM{                                        \
    1.0e-10}; /**< Medium precision epsilon static constant value. */          \
  static constexpr Real EPSILON_LOW{                                           \
    1.0e-08}; /**< Low precision epsilon static constant value. */             \
  static constexpr Real INFTY{                                                 \
    std::numeric_limits<Real>::infinity()};  /**< Infinity static constant     \
                                                value. */                      \
  static constexpr Real QUIET_NAN{                                             \
    std::numeric_limits<Real>::quiet_NaN()}; /**< Not-a-number static constant \
                                                value. */
#endif
...
namespace Sandals {
   ...
   * SANDALS_DEFAULT_INTEGER_TYPE. The default value is \c int.
   */
