...
namespace Sandals {
  ...
  struct Tableau {
    ...
    using VectorS = Eigen::Vector<Real, S>;
    using MatrixS = Eigen::Matrix<Real, S, S>;
    ...
    MatrixS A;           /**< MatrixS \f$ \mathbf{A} \f$. */
    VectorS b;           /**< Weights vector \f$ \mathbf{b} \f$. */
    VectorS b_e{VectorS::Zero()}; /**< Embedded weights vector \f$
                                     \hat{\mathbf{b}} \f$. */
    VectorS c;                    /**< Nodes vector \f$ \mathbf{c} \f$. */
    ...
    /**
     * Check the Butcher tableau order consistency for a generic Runge-Kutta
     * method.
     * \param[in] verbose Verbosity flag.
     * \return True if the Butcher tableau is consistent with its order, false
     * otherwise.
     */
...
#define CMD "Sandals::" << this->name << "::check(...): "
      ...
      if (type == Type::ERK &&
          (!this->A.isLowerTriangular() || !this->A.diagonal().isZero())) {
        SANDALS_ASSERT_WARNING(
            !verbose,
            CMD "A matrix occupancy not consistent with an ERK method.");
        return false;
      } else if (type == Type::DIRK && (!this->A.isLowerTriangular() ||
                                        this->A.diagonal().isZero())) {
        SANDALS_ASSERT_WARNING(
            !verbose,
            CMD "A matrix occupancy not consistent with a DIRK method.");
        return false;
      } else if (type == Type::IRK && this->A.isZero()) {
        SANDALS_ASSERT_WARNING(
            !verbose,
            CMD "A matrix occupancy not consistent with an IRK method.");
        return false;
      }
      ...
      if (this->order != computed_order) {
        SANDALS_ASSERT_WARNING(!verbose,
                               CMD "method order check failed, "
                                   << computed_order << " ≠ " << this->order
                                   << ".");
        return false;
      }
      ...
      Integer computed_order_e{
        compute_order(this->A, this->b_e, this->c, verbose)};
      if (this->order_e != computed_order_e) {
        SANDALS_ASSERT_WARNING(!verbose,
                               CMD "embedded method order check failed, "
                                   << computed_order_e << " ≠ " << this->order_e
                                   << ".");
        return false;
      } else {
        return true;
...
#undef CMD
    ...
    /**
     * Check the Butcher tableau order consistency for a generic symplectic
     * Runge-Kutta method.
     * \return True if the Butcher tableau is symplectic, false otherwise.
     */
    bool is_symplectic() const {
      return (b.asDiagonal() * A + A.transpose() * b.asDiagonal() -
              b * b.transpose())
                 .cwiseAbs()
                 .maxCoeff() < SQRT_EPSILON;
    }

    /**
     * Check the order of a Runge-Kutta method according to the conditions taken
     * from: *A family of embedded Runge-Kutta formulae*, J. R. Dormand and P.
     * J. Prince, Journal of Computational and Applied Mathematics, volume 6(1),
     * 1980. DOI 10.1016/0771-0509(80)90013-3.
     * \param[in] A MatrixS \f$ \mathbf{A} \f$.
     * \param[in] b Weights vector \f$ \mathbf{b} \f$ or embedded weights vector
     * \f$ \hat{\mathbf{b}} \f$.
     * \param[in] c Nodes vector \f$ \mathbf{c} \f$.
     * \param[in] verbose Verbosity flag.
     * \return The calculated order of the Runge-Kutta method.
     */
    Integer compute_order(const MatrixS &A,
                          const VectorS &b,
                          const VectorS &c,
...
#define CMD "Sandals::" << this->name << "::tableau_order(...): "
      ...
      Real tmp{(A * VectorS::Ones() - c).norm()};
      if (tmp > tolerance) {
        SANDALS_ASSERT_WARNING(
            !verbose,
            CMD "precheck failed, ||A*I - c|| = " << tmp << " ≠ 0.");
        return order;
      }
      ...
      if (std::abs(tmp - 1.0) > tolerance) {
        SANDALS_ASSERT_WARNING(
            !verbose,
            CMD "order 1 check failed, a_1 = " << tmp << " ≠ 1.");
        return order;
      }
      ...
      if (std::abs(tmp - 1.0 / 2.0) > tolerance) {
        SANDALS_ASSERT_WARNING(
            !verbose,
            CMD "order 2 check failed, a_1 = " << tmp << " ≠ 1/2.");
        return order;
      }
      ...
      // Check order 3
      VectorS c_2{c.array().pow(2)};
      tmp = b.transpose() * c_2;
      if (std::abs(tmp - 1.0 / 3.0) > tolerance) {
        SANDALS_ASSERT_WARNING(
            !verbose,
            CMD "order 3 check failed, a_1 = " << tmp << " ≠ 1/3.");
        return order;
      }
      ...
      if (std::abs(tmp - 1.0 / 6.0) > tolerance) {
        SANDALS_ASSERT_WARNING(
            !verbose,
            CMD "order 3 check failed, a_2 = " << tmp << " ≠ 1/6.");
        return order;
      }
      ...
      // Check order 4
      VectorS c_3{c.array().pow(3)};
      tmp = b.transpose() * c_3;
      if (std::abs(tmp - 1.0 / 4.0) > tolerance) {
        SANDALS_ASSERT_WARNING(
            !verbose,
            CMD "order 4 check failed, a_1 = " << tmp << " ≠ 1/4.");
        return order;
      }

      VectorS cAc{c.array() * Ac.array()};
      tmp = b.transpose() * cAc;
      if (std::abs(tmp - 1.0 / 8.0) > tolerance) {
        SANDALS_ASSERT_WARNING(
            !verbose,
            CMD "order 4 check failed, a_2 = " << tmp << " ≠ 1/8.");
        return order;
      }

      VectorS bA{(b.transpose() * A).transpose()};
      tmp = bA.transpose() * c_2;
      if (std::abs(tmp - 1.0 / 12.0) > tolerance) {
        SANDALS_ASSERT_WARNING(
            !verbose,
            CMD "order 4 check failed, a_3 = " << tmp << " ≠ 1/12.");
        return order;
      }
      ...
      if (std::abs(tmp - 1.0 / 24.0) > tolerance) {
        SANDALS_ASSERT_WARNING(
            !verbose,
            CMD "order 4 check failed, a_4 = " << tmp << " ≠ 1/24.");
        return order;
      }
      ...
      // Check order 5
      VectorS c_4{c.array().pow(4)};
      tmp = b.transpose() * c_4;
      if (std::abs(tmp - 1.0 / 5.0) > tolerance) {
        SANDALS_ASSERT_WARNING(
            !verbose,
            CMD "order 5 check failed, a_1 = " << tmp << " ≠ 1/5.");
        return order;
      }

      VectorS b_c2{b.array() * c_2.array()};
      tmp = b_c2.transpose() * Ac;
      if (std::abs(tmp - 1.0 / 10.0) > tolerance) {
        SANDALS_ASSERT_WARNING(
            !verbose,
            CMD "order 5 check failed, a_2 = " << tmp << " ≠ 1/10.");
        return order;
      }

      VectorS b_Ac{b.array() * Ac.array()};
      tmp = b_Ac.transpose() * Ac;
      if (std::abs(tmp - 1.0 / 20.0) > tolerance) {
        SANDALS_ASSERT_WARNING(
            !verbose,
            CMD "order 5 check failed, a_3 = " << tmp << " ≠ 1/20.");
        return order;
      }

      VectorS b_c{b.array() * c.array()};
      VectorS Ac2{A * c_2};
      tmp = b_c.transpose() * Ac2;
      if (std::abs(tmp - 1.0 / 15.0) > tolerance) {
        SANDALS_ASSERT_WARNING(
            !verbose,
            CMD "order 5 check failed, a_4 = " << tmp << " ≠ 1/15.");
        return order;
      }
      ...
      if (std::abs(tmp - 1.0 / 20.0) > tolerance) {
        SANDALS_ASSERT_WARNING(
            !verbose,
            CMD "order 5 check failed, a_5 = " << tmp << " ≠ 1/20.");
        return order;
      }
      ...
      if (std::abs(tmp - 1.0 / 30.0) > tolerance) {
        SANDALS_ASSERT_WARNING(
            !verbose,
            CMD "order 5 check failed, a_6 = " << tmp << " ≠ 1/30.");
        return order;
      }
      ...
      if (std::abs(tmp - 1.0 / 40.0) > tolerance) {
        SANDALS_ASSERT_WARNING(
            !verbose,
            CMD "order 5 check failed, a_7 = " << tmp << " ≠ 1/40.");
        return order;
      }
      ...
      if (std::abs(tmp - 1.0 / 60.0) > tolerance) {
        SANDALS_ASSERT_WARNING(
            !verbose,
            CMD "order 5 check failed, a_8 = " << tmp << " ≠ 1/60.");
        return order;
      }
      ...
      if (std::abs(tmp - 1.0 / 120.0) > tolerance) {
        SANDALS_ASSERT_WARNING(
            !verbose,
            CMD "order 5 check failed, a_9 = " << tmp << " ≠ 1/120.");
        return order;
      }
      ...
      // Check order 6
      VectorS c_5{c.array().pow(5)};
      tmp = b.transpose() * c_5;
      if (std::abs(tmp - 1.0 / 6.0) > tolerance) {
        SANDALS_ASSERT_WARNING(
            !verbose,
            CMD "order 6 check failed, a_1 = " << tmp << " ≠ 1/6.");
        return order;
      }
      ...
      if (std::abs(tmp - 1.0 / 12.0) > tolerance) {
        SANDALS_ASSERT_WARNING(
            !verbose,
            CMD "order 6 check failed, a_2 = " << tmp << " ≠ 1/12.");
        return order;
      }

      VectorS Ac_2{Ac.array().pow(2)};
      tmp = b_c.transpose() * Ac_2;
      if (std::abs(tmp - 1.0 / 24.0) > tolerance) {
        SANDALS_ASSERT_WARNING(
            !verbose,
            CMD "order 6 check failed, a_3 = " << tmp << " ≠ 1/24.");
        return order;
      }
      ...
      if (std::abs(tmp - 1.0 / 18.0) > tolerance) {
        SANDALS_ASSERT_WARNING(
            !verbose,
            CMD "order 6 check failed, a_4 = " << tmp << " ≠ 1/18.");
        return order;
      }
      ...
      if (std::abs(tmp - 1.0 / 36.0) > tolerance) {
        SANDALS_ASSERT_WARNING(
            !verbose,
            CMD "order 6 check failed, a_5 = " << tmp << " ≠ 1/36.");
        return order;
      }
      ...
      if (std::abs(tmp - 1.0 / 24.0) > tolerance) {
        SANDALS_ASSERT_WARNING(
            !verbose,
            CMD "order 6 check failed, a_6 = " << tmp << " ≠ 1/24.");
        return order;
      }
      ...
      if (std::abs(tmp - 1.0 / 30.0) > tolerance) {
        SANDALS_ASSERT_WARNING(
            !verbose,
            CMD "order 6 check failed, a_7 = " << tmp << " ≠ 1/30.");
        return order;
      }

      VectorS bc2A{A.transpose() * b_c2};
      tmp = bc2A.transpose() * Ac;
      if (std::abs(tmp - 1.0 / 36.0) > tolerance) {
        SANDALS_ASSERT_WARNING(
            !verbose,
            CMD "order 6 check failed, a_8 = " << tmp << " ≠ 1/36.");
        return order;
      }
      ...
      if (std::abs(tmp - 1.0 / 72.0) > tolerance) {
        SANDALS_ASSERT_WARNING(
            !verbose,
            CMD "order 6 check failed, a_9 = " << tmp << " ≠ 1/72.");
        return order;
      }

      VectorS bcA{b_c.transpose() * A};
      tmp = bcA.transpose() * cAc;
      if (std::abs(tmp - 1.0 / 48.0) > tolerance) {
        SANDALS_ASSERT_WARNING(
            !verbose,
            CMD "order 6 check failed, a_10 = " << tmp << " ≠ 1/48.");
        return order;
      }
      ...
      if (std::abs(tmp - 1.0 / 60.0) > tolerance) {
        SANDALS_ASSERT_WARNING(
            !verbose,
            CMD "order 6 check failed, a_11 = " << tmp << " ≠ 1/60.");
        return order;
      }
      ...
      if (std::abs(tmp - 1.0 / 120.0) > tolerance) {
        SANDALS_ASSERT_WARNING(
            !verbose,
            CMD "order 6 check failed, a_12 = " << tmp << " ≠ 1/120.");
        return order;
      }
      ...
      if (std::abs(tmp - 1.0 / 72.0) > tolerance) {
        SANDALS_ASSERT_WARNING(
            !verbose,
            CMD "order 6 check failed, a_13 = " << tmp << " ≠ 1/72.");
        return order;
      }

      VectorS bA_c{bA.array() * c.array()};
      tmp = bA_c.transpose() * Ac2;
      if (std::abs(tmp - 1.0 / 90.0) > tolerance) {
        SANDALS_ASSERT_WARNING(
            !verbose,
            CMD "order 6 check failed, a_14 = " << tmp << " ≠ 1/90.");
        return order;
      }
      ...
      if (std::abs(tmp - 1.0 / 120.0) > tolerance) {
        SANDALS_ASSERT_WARNING(
            !verbose,
            CMD "order 6 check failed, a_15 = " << tmp << " ≠ 1/120.");
        return order;
      }
      ...
      if (std::abs(tmp - 1.0 / 144.0) > tolerance) {
        SANDALS_ASSERT_WARNING(
            !verbose,
            CMD "order 6 check failed, a_16 = " << tmp << " ≠ 1/144.");
        return order;
      }
      ...
      if (std::abs(tmp - 1.0 / 180.0) > tolerance) {
        SANDALS_ASSERT_WARNING(
            !verbose,
            CMD "order 6 check failed, a_17 = " << tmp << " ≠ 1/180.");
        return order;
      }
      ...
      if (std::abs(tmp - 1.0 / 240.0) > tolerance) {
        SANDALS_ASSERT_WARNING(
            !verbose,
            CMD "order 6 check failed, a_18 = " << tmp << " ≠ 1/240.");
        return order;
      }
      ...
      if (std::abs(tmp - 1.0 / 360.0) > tolerance) {
        SANDALS_ASSERT_WARNING(
            !verbose,
            CMD "order 6 check failed, a_19 = " << tmp << " ≠ 1/360.");
        return order;
      }
      ...
      if (std::abs(tmp - 1.0 / 720.0) > tolerance) {
        SANDALS_ASSERT_WARNING(
            !verbose,
            CMD "order 6 check failed, a_20 = " << tmp << " ≠ 1/720.");
        return order;
      }

      order = 6;  // Order 6 is the highest order that can be checked with this
                  // method
