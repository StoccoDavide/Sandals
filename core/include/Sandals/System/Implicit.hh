...
#ifndef SANDALS_SYSTEM_IMPLICIT_HH
#define SANDALS_SYSTEM_IMPLICIT_HH
...
namespace Sandals {
  ...
  class Implicit {
   ...
   public:
    ...
    bool is_semiexplicit() const {
      return this->m_type == Type::SEMIEXPLICIT;
    }
     ...
     * \mathbf{x}^{\prime}, t) \f$.
     */
    virtual VectorF F(const VectorF &x,
                      const VectorF &x_dot,
     ...
     * \displaystyle\frac{
     * \partial\mathbf{F}(\mathbf{x}, \mathbf{x}^{\prime},
     ...
     * \mathbf{x}^{\prime}, t) \f$.
     */
    virtual MatrixJF JF_x(const VectorF &x,
                          const VectorF &x_dot,
                          const Real t) const = 0;
     ...
     * \mathbf{x}^{\prime}, t) \f$.
     */
    virtual MatrixJF JF_x_dot(const VectorF &x,
                              const VectorF &x_dot,
                              const Real t) const = 0;
     ...
     * t) \f$ is in the domain of the ODE/DAE system.
     * \param[in] x States \f$ \mathbf{x} \f$.
     * \param[in] t Independent variable (or time) \f$ t \f$.
  ...
  class ImplicitWrapper : public Implicit<Real, N, M> {
   ...
   private:
    ...
    FunctionJF m_JF_x{nullptr}; /**< Jacobian of the implicit ODE system
                                   function with respect to the states. */
    FunctionJF m_JF_x_dot{
      nullptr}; /**< Jacobian of the implicit ODE system function with respect
   ...
   public:
     ...
     * \param[in] t_JF_x The Jacobian of the implicit ODE system function with
     * respect to the states.
     * \param[in] t_JF_x_dot The Jacobian of the implicit ODE system function
     * with respect to the states derivative.
    ...
    /**
     * Get the Jacobian of the implicit ODE system function with respect to the
     * states.
     * \return The Jacobian of the implicit ODE system function with respect to
     * the states.
     */
    FunctionJF &JF_x() {
      return this->m_JF_x;
     ...
     * states derivative.
     * \return The Jacobian of the implicit ODE system function with respect to
     * the states derivative.
     */
    FunctionJF &JF_x_dot() {
      return this->m_JF_x_dot;
    }
...
#endif  // SANDALS_SYSTEM_IMPLICIT_HH
