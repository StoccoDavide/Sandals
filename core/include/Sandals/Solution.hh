...
namespace Sandals {
   ...
   *
   * Class container for the numerical solution of a system of Ordinary
   * Differential Equations (ODEs) or Differential Algebraic Equations (DAEs).
   * The solution is stored in a independent variable (or time) vector, states
   * matrix and invariant matrix, where each row of matrix represents the value
   * of a state/invariant at a given mesh node, *i.e.*,
   *
   * \f[
   * \begin{array}{l}
   *   \mathbf{t} &=& \left[ t_1, t_2, \ldots, t_k \right]^\top \text{,} \\
   *   \mathbf{X} &=& \begin{bmatrix} \mathbf{x}_1 \\ \mathbf{x}_2 \\ \vdots
   ...
   * \text{,} \\
   *   \mathbf{H} &=& \begin{bmatrix} \mathbf{h}_1 \\ \mathbf{h}_2 \\ \vdots
   * \\ \mathbf{h}_k \end{bmatrix}
   *   =  \begin{bmatrix} h_{11} & h_{12} & \cdots & h_{1k} \\
   *                      h_{21} & h_{22} & \cdots & h_{2k} \\
   *                      \vdots & \vdots & \ddots & \vdots \\
   *                      h_{m1} & h_{m2} & \cdots & h_{mk} \end{bmatrix}
   * \text{.}
   * \end{array}
   * \f]
   *
   * Such a solution can be retrieved as standard library vectors or Eigen
   * library vectors.
  ...
  struct Solution {
     ...
     * (or time) and states sizes.
     * \param[in] size The size of the independent variable (or time) vector and
     * states matrix.
     */
    ...
    /**
     * Resize conserving the data of the independent variable (or time) vector
     * and states matrix.
     * \param[in] size The size of the independent variable (or time) vector and
     * states matrix.
     */
    ...
    /**
     * Check if the independent variable (or time) vector and states matrix are
     * empty.
     * \return True if the independent variable (or time) vector and states
     * matrix are empty, false otherwise.
     */
    bool is_empty() const {
    ...
    /**
     * Get the size of the independent variable (or time) vector and states
     * matrix.
     * \return The size of the independent variable (or time) vector and states
     * matrix.
     */
    Integer size() const {
      return this->t.size();
    ...
    /**
     * Get the independent variable (or time) vector \f$ \mathbf{t} \f$ as a
     * standard library vector.
     * \return The independent variable (or time) vector \f$ \mathbf{t} \f$ as a
     * standard library vector.
     */
    ...
    /**
     * Get the independent variable (or time) vector \f$ \mathbf{t} \f$ as Eigen
     * library vector.
     * \return The independent variable (or time) vector \f$ \mathbf{t} \f$ as
     * Eigen library vector.
     */
    Vector eigen_t() const {
      return this->t;
    }

    /**
     * Get the \f$ i \f$-th state vector \f$ \mathbf{x}_i \f$ as a standard
     * library vector.
    ...
    /**
     * Get the i\f$ i \f$-th state vector \f$ \mathbf{x}_i \f$ as a Eigen
     * library vector.
     * \param[in] i The index of the state vector.
    ...
    /**
     * Get a map of the states vectors \f$ \mathbf{X} \f$ as standard library
     * vectors.
    ...
    std::map<Integer, std::vector<Real>> std_x() const {
      ...
      for (Integer i{0}; i < N; ++i) {
        x_map[i] = this->std_x(i);
      }
      return x_map;
    }

    /**
     * Get a map of the states vectors \f$ \mathbf{X} \f$ as Eigen library
     * vectors.
    ...
    std::map<Integer, Vector> eigen_x() const {
      ...
      for (Integer i{0}; i < N; ++i) {
        x_map[i] = this->x.row(i).transpose().eval();
      }
      return x_map;
    }
     ...
     */
    std::map<std::string, std::vector<Real>> std_x(
        const std::vector<std::string> &names) const {
      std::map<std::string, std::vector<Real>> x_map;
      for (Integer i{0}; i < N; ++i) {
        x_map[names[i]] = this->std_x(i);
      }
      return x_map;
    }
     ...
     * \return A map of the states vectors as Eigen library vectors.
     */
    std::map<std::string, Vector> eigen_x(
        const std::vector<std::string> &names) const {
      std::map<std::string, Vector> x_map;
      for (Integer i{0}; i < N; ++i) {
        x_map[names[i]] = this->x.row(i).transpose().eval();
      }
      return x_map;
    }

    /**
     * Get the \f$ i \f$-th invariants vector \f$ \mathbf{h}_i \f$ as a standard
     * library vector.
     * \param[in] i The index of the invariant vector.
     * \return The \f$ i \f$-th invariants vector as a standard library vector.
     */
    std::vector<Real> std_h(const Integer i) const {
      Vector tmp(this->h.row(i));
      return std::vector<Real>(tmp.data(), tmp.data() + tmp.size());
    }

    /**
     * Get the \f$ i \f$-th invariants vector \f$ \mathbf{h}_i \f$ as a Eigen
     * library vector.
     * \param[in] i The index of the invariant vector.
     * \return The \f$ i \f$-th invariants vector as a Eigen library vector.
     */
    Vector eigen_h(const Integer i) const {
      return this->h.row(i).transpose().eval();
    }

    /**
     * Get a map of the invariants vectors \f$ \mathbf{H} \f$ as standard
     * library vectors.
    ...
    std::map<Integer, std::vector<Real>> std_h() const {
      ...
      for (Integer i{0}; i < M; ++i) {
        h_map[i] = this->std_h(i);
      }
      return h_map;
    }

    /**
     * Get a map of the invariants vectors \f$ \mathbf{H} \f$ as Eigen library
     * vectors.
    ...
    std::map<Integer, Vector> eigen_h() const {
      ...
      for (Integer i{0}; i < M; ++i) {
        h_map[i] = this->h.row(i).transpose().eval();
      }
      return h_map;
    }
     ...
     */
    std::map<std::string, std::vector<Real>> std_h(
        const std::vector<std::string> &names) const {
      std::map<std::string, std::vector<Real>> h_map;
      for (Integer i{0}; i < M; ++i) {
        h_map[names[i]] = this->std_h(i);
      }
      return h_map;
    }
     ...
     * \return A map of the invariants vectors as Eigen library vectors.
     */
    std::map<std::string, Vector> eigen_h(
        const std::vector<std::string> &names) const {
      std::map<std::string, Vector> h_map;
      for (Integer i{0}; i < M; ++i) {
        h_map[names[i]] = this->h.row(i).transpose().eval();
      }
      return h_map;
    }
  };
