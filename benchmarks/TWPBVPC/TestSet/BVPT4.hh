...
public:
  BVPT4Explicit() : Explicit<Real, 2, 0>("BVPT4Explicit") {}
  ...
  {
    VectorF f;
    f << x(1), 1.0/this->m_lambda * (-x(1) + (1.0 + this->m_lambda)*x(0));
    return f;
  }
  ...
  MatrixJF Jf_x(VectorF const & /*x*/, Real const /*t*/) const override {
    ...
    Jf_x(0, 1) = 1.0;
    Jf_x(1, 0) = 1.0/this->m_lambda * (1.0 + this->m_lambda);
...
public:
  BVPT4Implicit() : Implicit<Real, 2, 0>("BVPT4Implicit") {}
  ...
  {
    VectorF F;
    F << x_dot(0) - x(1), x_dot(1) - 1.0/this->m_lambda * (-x(1) + (1.0 + this->m_lambda)*x(0));
    return F;
  }
...
public:
  BVPT4SemiExplicit() : SemiExplicit<Real, 2, 0>("BVPT4SemiExplicit") {}
  ...
  {
    VectorB b;
    b << x(1), 1.0/this->m_lambda * (-x(1) + (1.0 + this->m_lambda)*x(0));
    return b;
  }
  ...
  MatrixJB Jb_x(VectorF const & /*x*/, Real const /*t*/) const override {
    ...
    Jb_x(0, 1) = 1.0;
    Jb_x(1, 0) = 1.0/this->m_lambda * (1.0 + this->m_lambda);
...
class BVPT4Problem : public Problem<Real, 2, 0, Integrator>
{
public:
  ...
  BVPT4Problem()
    : Problem<Real, 2, 0, Integrator>("BVPT4Problem", std::make_unique<System>(),
  ...
  {
    VectorF b;
    Real const lambda{this->lambda()};
    b << x_ini(0) - 1.0 - std::exp(-2.0), x_end(0) - 1.0 - std::exp(-2.0*(1.0 + lambda)/lambda);
    return b;
  }
  ...
  VectorX exact_solution(VectorX const & t) const {
    ...
    for (Integer i{0}; i < t.size(); ++i) {
      x(i) = BVPT4Problem<Real, System, Integrator>::exact_solution(t(i));
    }
    return x;
  }
